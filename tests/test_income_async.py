...
class TestIncomeAPI:
    ...
    @pytest.mark.asyncio
    async def test_create_single_item_success(self, authenticated_client, income_response):
    ...
    @pytest.mark.asyncio
    async def test_create_multiple_items_success(self, authenticated_client, income_response):
        ...
        with respx.mock(base_url="https://lknpd.nalog.ru/api/v1") as respx_mock:
            ...
            services = [
                IncomeServiceItem(name="Service 1", amount=Decimal("100.50"), quantity=Decimal("1")),
                IncomeServiceItem(name="Service 2", amount=Decimal("200.25"), quantity=Decimal("2")),
            ]
    ...
    @pytest.mark.asyncio
    async def test_create_with_custom_client(self, authenticated_client, income_response):
        ...
        with respx.mock(base_url="https://lknpd.nalog.ru/api/v1") as respx_mock:
            ...
            income_api = client.income()
            result = await income_api.create("Test Service", 100, 1, client=custom_client)
    ...
    @pytest.mark.asyncio
    async def test_total_amount_calculation(self, authenticated_client, income_response):
        ...
        with respx.mock(base_url="https://lknpd.nalog.ru/api/v1") as respx_mock:
            ...
            services = [
                IncomeServiceItem(name="Service 1", amount=Decimal("100.50"), quantity=Decimal("2")),  # 201.00
                IncomeServiceItem(name="Service 2", amount=Decimal("50.25"), quantity=Decimal("3")),   # 150.75
            ]
    ...
    @pytest.mark.asyncio
    async def test_create_legal_entity_validation_success(self, authenticated_client, income_response):
    ...
    @pytest.mark.asyncio
    async def test_create_legal_entity_validation_missing_inn(self, authenticated_client):
        ...
        with respx.mock(base_url="https://lknpd.nalog.ru/api/v1"):
            with pytest.raises(ValueError, match="Client INN cannot be empty for legal entity"):
                await income_api.create("Service", 100, 1, client=legal_client)

    @pytest.mark.asyncio
    async def test_create_legal_entity_validation_missing_display_name(self, authenticated_client):
        ...
        # Mock HTTP to avoid real requests - we're testing validation logic
        with respx.mock(base_url="https://lknpd.nalog.ru/api/v1"):
            with pytest.raises(ValueError, match="Client DisplayName cannot be empty for legal entity"):
                await income_api.create("Service", 100, 1, client=legal_client)
    ...
    async def test_cancel_success(self, authenticated_client, cancel_response):
        ...
        with respx.mock(base_url="https://lknpd.nalog.ru/api/v1") as respx_mock:
            ...
            result = await income_api.cancel(
                ...
                CancelCommentType.REFUND
            )
            
            assert result["incomeInfo"]["approvedReceiptUuid"] == "test-receipt-uuid-123"
            assert result["incomeInfo"]["cancellationInfo"]["comment"] == "Возврат средств"

    @pytest.mark.asyncio
    async def test_cancel_with_string_comment(self, authenticated_client, cancel_response):
...
class TestIncomeServiceItem:
    ...
    def test_service_item_validation_negative_amount(self):
        ...
        with pytest.raises(ValidationError, match="Input should be greater than 0"):
            IncomeServiceItem(name="Service", amount=Decimal("-100"), quantity=Decimal("1"))

    def test_service_item_validation_zero_quantity(self):
        """Test validation error for zero quantity."""
        with pytest.raises(ValidationError, match="Input should be greater than 0"):
            IncomeServiceItem(name="Service", amount=Decimal("100"), quantity=Decimal("0"))
