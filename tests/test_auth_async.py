...
class TestAuthProviderImpl:
    ...
    async def test_create_new_access_token_success(self, sample_token_response):
        ...
        with respx.mock(base_url="https://lknpd.nalog.ru/api/v1") as respx_mock:
            ...
            token_json = await auth_provider.create_new_access_token("test_inn", "test_password")
    ...
    async def test_create_new_access_token_unauthorized(self):
        ...
        with respx.mock(base_url="https://lknpd.nalog.ru/api/v1") as respx_mock:
            respx_mock.post("/auth/lkfl").mock(
                return_value=httpx.Response(401, text=json.dumps({"message": "Invalid credentials"}))
            )
            
            auth_provider = AuthProviderImpl()
            
            with pytest.raises(UnauthorizedException) as exc_info:
                await auth_provider.create_new_access_token("invalid_inn", "invalid_password")
    ...
    @pytest.mark.asyncio
    async def test_create_new_access_token_by_phone_success(self, sample_token_response):
...
class TestClient:
    ...
    async def test_create_new_access_token(self, sample_token_response):
        ...
        with respx.mock(base_url="https://lknpd.nalog.ru/api/v1") as respx_mock:
            ...
            client = Client()
            token_json = await client.create_new_access_token("test_inn", "test_password")
    ...
    async def test_401_triggers_refresh_and_retry(self, sample_token_response):
        ...
        with respx.mock(base_url="https://lknpd.nalog.ru/api/v1") as respx_mock:
            ...
            income_mock.side_effect = [
                ...
                httpx.Response(200, json={"approvedReceiptUuid": "test-uuid"})  # After refresh
            ]
